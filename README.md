...
FindSurface's error code other than  `FS_NOT_FOUND` means it failed to execute its algorithm for any reason (e.g., an invalid parameter value, lack of memory). It is recommended to design your application defensively so that your application does not have to get any error except the "out of memory" case in run-time. Refer to [here](TBD) for the cases of when FindSurface throws an error message.
